...
# 目标函数，其最优解为 100
fitness_func = eas.target.f9

# 参数项
upperxs = np.array([5.12] * n) # 向量各分量上限
lowerxs = np.array([-5.12] * n) # 向量各分量下限

# 开始
time_str = time.strftime('%Y-%m-%d-%H-%M-%S',time.localtime(time.time()))
log_file = open(
    './storages/logs/DE-function-09-%s-summary.tsv' % time_str,
    mode='ab')
...
for i in range(times):
    ...
    factors = {
        'cr': eas.factor.ConstantFactor(0.3, gen),
        'f': eas.factor.ConstantFactor(0.5, gen),
    }

    algo = DE(_np, n, upperxs, lowerxs,
              factors=factors,
